...
%\usepackage[opticals, onlytext]{MinionPro}
...
%\subtitle{Lesson learnt}%Review of what has been done so far}
\date{\today}
...
%\begin{frame}
%	\begin{itemize}
%		\item Heavily shaped by \emph{functional programming} concepts (map-filter-reduce, stateless, monadic behaviour of observables, referencial transparency-\emph{ish} style) 
%		\item \emph{Not just FP}: it does support 
%			\emph{side effects} (doOnSomething, Subjects, etc) 
%		\item \emph{Push-based} (ie, reactive)
%		\item \emph{Declarative scheduling}: abstraction on top of synchornous vs asynchronous behavior (inversion of control, the consumer specifies it) 
%	\end{itemize}
%\end{frame}
...
%	\begin{figure}
%		\centering
%		\includegraphics[width=\textwidth]{backus}
%	\end{figure}
%\end{frame}
%\begin{frame}[fragile]
%	\frametitle{Non-mathematical functions}
%	Side effects, for example: 
%	\begin{itemize}
%		\item Mutations: \verb|i++|
%		\item Changing boolean flags, eg \verb|i++| is logged in
%		\item Logs,
%		\item Exceptions,
%		\item Notion of asynchrnous vs synchronous 
%	\end{itemize}
%\end{frame}
\begin{frame}
	FP ``keywords'':
